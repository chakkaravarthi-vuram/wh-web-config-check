...
(function () {
    ...
    const titleSelectableClass = 'tox-collection__item-title';
    ...
    const titleClass = 'tox-collection__item-labeltitle';
    ...
    const renderTitleText = text => ({
      dom: {
        tag: 'div',
        classes: [textClass, titleClass]
      },
      components: [text$1(global$8.translate(text))]
    });
    ...
    const renderItemDomStructure = (ariaLabel, customClasses) => {
      const menuClasses = customClasses || [
        navClass,
        selectableClass
      ];
    ...
    const renderNormalItemStructure = (info, providersBackstage, renderIcons, fallbackIcon) => {
      ...
      const customTextRenderer = info.isTitleMenuItem ? renderTitleText : renderText;
      const textRender = Optional.from(info.meta).fold(() => customTextRenderer, meta => has$2(meta, 'style') ? curry(renderStyledText, meta.style) : customTextRenderer);
      const content = info.htmlContent.fold(() => info.textContent.map(textRender), html => Optional.some(renderHtml(html, textCustomClasses)));
      const customMenuClasses = info.isTitleMenuItem ? [
        navClass,
        selectableClass,
        titleSelectableClass,
      ] : null;
      const menuItem = {
        dom: renderItemDomStructure(info.ariaLabel, customMenuClasses),
        optComponents: [
          leftIcon,
          content,
    ...
    const renderNormalItem = (spec, itemResponse, providersBackstage, renderIcons = true, isTitleMenuItem) => {
      const getApi = component => ({
        isEnabled: () => !Disabling.isDisabled(component),
        setEnabled: state => Disabling.set(component, !state)
      });
      const structure = renderItemStructure({
        ...
        checkMark: Optional.none(),
        shortcutContent: spec.shortcut,
        isTitleMenuItem: isTitleMenuItem,
      }, providersBackstage, renderIcons);
      return renderCommonItem({
        data: buildData(spec),
        getApi,
    ...
    const createMenuItemFromBridge = (item, itemResponse, backstage, menuHasIcons, isHorizontalMenu) => {
      ...
      switch (item.type) {
      case 'menuitem':
        return createMenuItem(item).fold(handleError, d => Optional.some(normal(parseForHorizontalMenu(d), itemResponse, providersBackstage, menuHasIcons, item.isTitleMenuItem)));
      case 'nestedmenuitem':
