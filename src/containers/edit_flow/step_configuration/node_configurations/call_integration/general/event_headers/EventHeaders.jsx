...
let cancelTokenForEventHeaders = null;

const setCancelTokenForEventHeaders = (c) => {
  cancelTokenForEventHeaders = c;
};

function EventHeaders(props) {
  const { steps, metaData, systemFieldParams } = props;

  const { t } = useTranslation();

  const { EVENT_HEADERS } = CALL_INTEGRATION_STRINGS(t).GENERAL.MAPPING;
    ...
    dispatch(
      nodeConfigDataChange({
        [RESPONSE_FIELD_KEYS.EVENT_HEADERS]:
          data?.[RESPONSE_FIELD_KEYS.EVENT_HEADERS],
        [RESPONSE_FIELD_KEYS.EVENT_HEADERS_ERROR]:
          data?.[RESPONSE_FIELD_KEYS.EVENT_HEADERS_ERROR],
      }),
    );
  };
  ...
  return (
    <div className={gClasses.MT12}>
      <Text
        ...
        content={EVENT_HEADERS.TITLE}
      />
      <FieldMappingTable
        key={RESPONSE_FIELD_KEYS.EVENT_HEADERS}
        initialRawData={eventInternalHeaders}
        mappedServerData={serverEventHeaders}
        mappedData={eventHeaders}
        fieldDetails={fieldDetails}
        tableHeaders={EVENT_HEADERS.TABLE_HEADERS}
        ...
        mappingListKey={RESPONSE_FIELD_KEYS.EVENT_HEADERS}
        errorListKey={RESPONSE_FIELD_KEYS.EVENT_HEADERS_ERROR}
        handleMappingChange={handleMappingChange}
        parentId={metaData?.flowId}
        errorList={eventHeaderErrorList}
        parentSetCancelToken={setCancelTokenForEventHeaders}
        parentCancelToken={cancelTokenForEventHeaders}
      />
    </div>
  );
}
