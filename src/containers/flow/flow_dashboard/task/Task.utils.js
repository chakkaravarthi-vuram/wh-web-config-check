...
export const getTaskAPIData = (state, formStatus, flow, setCollectData = false, collect_data = null) => {
  const taskPostData = {};
    ...
    if (state.auto_trigger_details.is_recursive) {
      ...
      if (state.auto_trigger_details.recursive_data.type === 'day') {
        ...
        if (taskPostData.auto_trigger_details.recursive_data.repeat_type === 'selected_date') {
          ...
          const [onDayData] = state.auto_trigger_details.recursive_data.on_day;
          taskPostData.auto_trigger_details.recursive_data.on_day = onDayData;
        }
      }
    }
  }
  // taskPostData.flow_uuid = flow.flowUuid;
  taskPostData.instance_id = flow.flowInstanceId;
