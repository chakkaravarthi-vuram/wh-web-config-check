pipeline {
  ...
  stages {
    stage('Load Environment Variables') {
      steps {
        ...
        script {
          ...
          env.PROFILE = sh(returnStdout: true, script: "node get_props.js PROFILE $ENVIRONMENT").trim()        
    ...
    stage('Deploy to ECS Production') {
      ...
      steps {
        sh "aws s3 cp --recursive s3://jenkins-server-configs/$ACTIVE_JOB/production ."
        withCredentials([string(credentialsId: 'docker_conn_string', variable: 'PW1'),string(credentialsId: 'code-deploy-access-aws', variable: 'access'),string(credentialsId: 'code-deploy-secret-aws', variable: 'secret')]) {          
        sh "sh ./deploy_to_ecs_prod.sh"
